...
class RankBasedSampler(BaseSampler):

    def __init__(self, data, reweighting, *args, **kwargs):
        super(RankBasedSampler, self).__init__(data, reweighting)
    ...
    def compute_rank(self, measurements, inverse=False):
        """
        compute rand and apply scale factor as well as power factor
        :param inverse: to inverse the rank
        :return: rank
        """

        input_shape = measurements.shape

        # -----------
        # compute rank
        # -----------
        measure_rank = np.array(pd.Series(measurements.reshape(-1)).rank())
        if inverse:
            measure_rank = np.max(measure_rank) + 1 - measure_rank

        # -----------
        # scale & pow
        # -----------

        measure_rank *= self.scale_factor
        measure_rank = np.power(measure_rank, self.pow_factor)

        measure_rank = measure_rank.reshape(input_shape)
...
@sampler_decorator
class SamplerWithNDMIS(InterpolationSampler, RankBasedSampler):
    """
    Implement of NDMIS (Non-metric Dynamic Mesh-based Importance Sampling)
    """
    ...
    def __init__(
        ...
        super(SamplerWithNDMIS, self).__init__(data, reweighting)
        ...
        self.scale_factor = kwargs["rank_scale_factor"]
        self.pow_factor = kwargs["pow_scale_factor"]
    ...
    def compute_scores(self):
        ...
        # to rank
        interp_scores = self.compute_rank(interp_scores)
